...
$assignments = $conn->query("SELECT * FROM assignments WHERE deadline >= CURDATE() ORDER BY deadline");

// Get user's submissions (assignment_id is now title)
$submissions = [];
$stmt = $conn->prepare("SELECT assignment_id, file_name FROM student_assignments WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        if (move_uploaded_file($fileTmpName, $filePath)) {
            // Save to database
            $stmt = $conn->prepare("INSERT INTO student_assignments (user_id, assignment_id, file_name, file_path) VALUES (?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_file'])) {
    ...
    // Get file path
    $stmt = $conn->prepare("SELECT file_path FROM student_assignments WHERE user_id = ? AND assignment_id = ?");
    ...
    // Delete record
    $stmt = $conn->prepare("DELETE FROM student_assignments WHERE user_id = ? AND assignment_id = ?");
    $stmt->bind_param("ss", $user_id, $assignmentTitle);
    $stmt->execute();
    $stmt->close();
