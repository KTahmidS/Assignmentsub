...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_assignment'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO assignmentsit (title, description, deadline) VALUES (?, ?, ?)");
    ...
    if (isset($_POST['delete_assignment'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM student_assignmentsit WHERE assignment_id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->close();
        
        // Then delete the assignment itself
        $stmt = $conn->prepare("DELETE FROM student_assignmentsit WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->close();
    }
...
// Get all assignments
$assignments = $conn->query("SELECT * FROM assignmentsit ORDER BY deadline");
...
$submissions = $conn->query("
    ...
    FROM student_assignmentsit sa
    LEFT JOIN users u ON sa.user_id = u.username  /* Changed to match username as user_id */
    LEFT JOIN assignmentsit a ON sa.assignment_id = a.id
